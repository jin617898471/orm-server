...
//			queryParams : {queryCondition : ""},
			...
			columns:[[
				...
				{field:'operate',title:'操作',align:'center',
					formatter: function(value,row,index){
						return '<a href="javascript:void(0)" class="operate-items operate-items-edit" onclick="editUser(\''+row.userId +'\')">编辑</a>' +
								'<a href="javascript:void(0)" class="operate-items" onclick="deleteUser(\''+row.userId +'\')">删除</a>';
					}
				}
			]],
			pagination : true,
