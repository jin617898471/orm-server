			...
			data = eleArray.map(function (i, elem) {
				...
				if (elem.type == "checkbox") {
		        	if (this.ucheckvalue) {
		            	...
		            	if ($("input[type=checkbox][name=" + elem.name + "]:checked").length <= 0 && $.inArray(elem.name, _temparray) < 0) {
		                	val = "";
		                	_temparray.push(elem.name);
		            	}else{
		        	...
		        	if ($("input[type=radio][name=" + elem.name + "]:checked").length <= 0 && $.inArray(elem.name, _temparray) < 0) {
		            	val = "";
		            	_temparray.push(elem.name);
		        	}else{
		            	val = elem.checked ? val : null;
